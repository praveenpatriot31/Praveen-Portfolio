      ...
      <div className="relative z-10 mx-auto max-w-[1500px]">
        ...
        <div className="grid gap-5 py-8 sm:py-10 lg:grid-cols-12 lg:py-12">
          ...
          <div className="flex flex-col justify-between rounded-[24px] bg-[#0F1E3A] p-7 text-white sm:p-10 lg:col-span-5">
            <div><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-white/45">Project snapshot</p><h3 className="mt-4 text-4xl font-black uppercase leading-[0.92] tracking-[-0.04em] sm:text-5xl">Colours a<br />brighter<br /><span className="text-[#FF8A00]">tomorrow.</span></h3><p className="mt-6 text-sm leading-7 text-white/65">The supplied 10-page portfolio documents the brand direction from strategy and identity through packaging, digital, signage, retail and supporting applications.</p></div>
        ...
        <div className="mt-8"><div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#1161E7]">01 / Packaging</p><h3 className="mt-2 text-3xl font-black uppercase tracking-[-0.04em] sm:text-4xl">Product system.</h3></div><span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#0F1E3A]/40">Supplied source · page 07</span></div><SourceImage file="07-packaging.webp" alt="Paint-O Paints packaging system and product range from supplied source" /></div>

        <div className="mt-8"><div className="mb-5"><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#1161E7]">02 / Digital & social</p><h3 className="mt-2 text-3xl font-black uppercase tracking-[-0.04em] sm:text-4xl">Brand communication in digital spaces.</h3></div><div className="grid gap-5 md:grid-cols-2">{VISUALS.slice(1).map(([file, title, description]) => <article key={file}><SourceImage file={file} alt={`Paint-O Paints ${title.toLowerCase()} application visual from supplied source`} /><div className="pt-4"><h4 className="text-lg font-bold">{title}</h4><p className="mt-1 text-sm leading-6 text-[#0F1E3A]/55">{description}</p></div></article>)}</div></div>
