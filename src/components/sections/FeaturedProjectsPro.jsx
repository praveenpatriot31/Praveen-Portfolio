...
const PROJECTS = [
  { id: "01", title: "Paint-O Paints", category: "BRAND IDENTITY", type: "Visual System & Guidelines", visual: "paint-o" },
  { id: "02", title: "Studio Identity & Layouts", category: "VISUAL DIRECTION", type: "Brand Systems", visual: "identity" },
  { id: "03", title: "Vector Marketing Assets", category: "GRAPHIC DESIGN", type: "Marketing Communication", visual: "vector" },
];
...
function PaintOVisual() {
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#F3F4F6]">
      ...
      <div className="absolute bottom-8 left-8 flex gap-2">{COLORS.map((color) => <span key={color} className="h-2 w-10 rounded-full" style={{ backgroundColor: color }} />)}</div>
...
function ArchiveVisual({ variant }) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-zinc-900">
      ...
      <div className="absolute left-8 top-10 text-[9px] font-mono uppercase tracking-[0.28em] text-zinc-500">{identity ? "IDENTITY / LAYOUT" : "VECTOR / COLLATERAL"}</div>
      <div className="absolute left-8 right-8 top-1/2 -translate-y-1/2 text-[clamp(2.5rem,7vw,6rem)] font-black uppercase leading-[0.8] tracking-[-0.06em] text-white">
        {identity ? <>Studio<br /><span className="text-[#FF453A]">System</span></> : <>Vector<br /><span className="text-zinc-500">Assets</span></>}
      </div>
      <div className="absolute bottom-8 left-8 flex gap-1.5"><span className="h-1.5 w-8 bg-[#FF453A]" /><span className="h-1.5 w-4 bg-white/30" /><span className="h-1.5 w-2 bg-white/15" /></div>
    </div>
  );
}
    ...
    <section id="work" className="relative overflow-hidden border-t border-white/[0.06] bg-[#050505] px-6 py-28 text-white md:px-12 lg:py-36 xl:px-24">
      <div className="relative z-10 mx-auto max-w-[1500px]">
        <header className="mb-16 flex flex-col justify-between gap-8 border-b border-white/10 pb-10 md:flex-row md:items-end lg:mb-20">
          ...
          <p className="max-w-md text-sm leading-7 text-zinc-400 md:text-base">A focused selection of visual design, brand communication and creative production work.</p>
        </header>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          {PROJECTS.map((project, index) => (
            <article key={project.id} className={`group block rounded-[26px] border border-white/[0.08] bg-white/[0.025] p-3 md:p-4 ${index === 0 ? "lg:col-span-12" : "lg:col-span-6"}`}>
              <div className={`relative overflow-hidden rounded-[19px] bg-zinc-950 ${index === 0 ? "aspect-[16/7]" : "aspect-[4/3]"}`}>
                ...
                <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/10 bg-black/55 px-3 py-1.5 backdrop-blur-md"><span className="text-[9px] font-mono tracking-[0.18em] text-[#FF453A]">{project.id}</span><span className="h-1 w-1 rounded-full bg-white/30" /><span className="text-[9px] font-mono tracking-[0.14em] text-white/60">SELECTED WORK</span></div>
                <div className={`absolute bottom-5 left-5 right-5 md:bottom-7 md:left-7 md:right-7 ${project.visual === "paint-o" ? "text-[#0F1E3A]" : "text-white"}`}>
                  <span className={`mb-2 block text-[9px] font-bold uppercase tracking-[0.25em] ${project.visual === "paint-o" ? "text-[#1161E7]" : "text-[#FF453A]"}`}>{project.category}</span>
                  <h3 className={`font-black uppercase leading-none tracking-[-0.035em] ${index === 0 ? "text-3xl sm:text-5xl lg:text-6xl" : "text-2xl sm:text-3xl"}`}>{project.title}</h3>
                  <p className={`mt-3 text-[10px] font-mono uppercase tracking-[0.14em] ${project.visual === "paint-o" ? "text-[#0F1E3A]/50" : "text-white/50"}`}>{project.type}</p>
                </div>
              </div>
            </article>
          ))}
